...
import com.triggerapp.domain.model.AuthUser
...
class AuthRepositoryImpl(
    ...
    override suspend fun signUp(username: String, email: String, password: String): Result<Unit> =
        try {
            ...
            val uid = result.user?.uid ?: error(TriggerStrings.Errors.NO_UID_AFTER_SIGNUP)
            userRepository
                .createUserProfile(
                    userId = uid,
                    username = username,
                    ...
                    imageUrl = TriggerStrings.Defaults.PROFILE_IMAGE,
                )
                .getOrElse { throw it }
            Result.success(Unit)
        } catch (e: Exception) {
