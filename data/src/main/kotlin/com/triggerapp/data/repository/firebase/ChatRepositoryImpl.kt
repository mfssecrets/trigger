...
import com.triggerapp.data.mapper.toChatMessageOrNull
import com.triggerapp.domain.model.ChatListEntry
...
class ChatRepositoryImpl(
    ...
    private val chatsRef: DatabaseReference = database.reference.child(TriggerStrings.Db.NODE_CHATS)
    private val chatListRef: DatabaseReference = database.reference.child(TriggerStrings.Db.NODE_CHAT_LIST)
        ...
        val listener = object : ValueEventListener {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                val ids = snapshot.children.mapNotNull {
                    it.child(TriggerStrings.Db.CHILD_ID).getValue(String::class.java)
                }
                trySend(ids)
            }
    ...
    override suspend fun fetchChatListPage(
        ...
        val base = chatListRef.child(currentUserId).orderByChild(TriggerStrings.Db.CHILD_TIMESTAMP)
        ...
        val rows = snap.children.mapNotNull { child ->
            ...
            val idField = child.child(TriggerStrings.Db.CHILD_ID).getValue(String::class.java) ?: partnerId
            val ts = child.child(TriggerStrings.Db.CHILD_TIMESTAMP).getValue(String::class.java) ?: "0"
    ...
    override suspend fun sendMessage(
        ...
        val map = mapOf(
            TriggerStrings.Db.CHILD_RECEIVER_ID to receiverId,
            TriggerStrings.Db.CHILD_SENDER_ID to senderId,
            TriggerStrings.Db.CHILD_MESSAGE to message,
            TriggerStrings.Db.CHILD_TIMESTAMP to timestamp,
            TriggerStrings.Db.CHILD_SEEN to false,
        )
    ...
    private suspend fun ensureChatListEntry(ownerId: String, otherId: String, timestamp: String) {
        ...
        if (!snap.exists()) {
            ref.child(TriggerStrings.Db.CHILD_ID).setValue(otherId).await()
            ref.child(TriggerStrings.Db.CHILD_TIMESTAMP).setValue(timestamp).await()
        }
    }
        ...
        runCatching {
            ...
            for (child in snap.children) {
                ...
                if (msg.senderId == peerId && msg.receiverId == myUserId && !msg.seen) {
                    child.ref.child(TriggerStrings.Db.CHILD_SEEN).setValue(true).await()
                }
            }
        }
}
