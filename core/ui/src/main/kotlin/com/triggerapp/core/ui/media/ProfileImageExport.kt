package com.triggerapp.core.ui.media
...
suspend fun Context.profileImageToShareJpegFile(imageUrl: String): File? = withContext(Dispatchers.IO) {
    ...
    val out = File(dir, "trigger_share_${System.currentTimeMillis()}.jpg")
...
fun Context.shareProfileImageFile(file: File) {
    ...
    startActivity(Intent.createChooser(intent, TriggerStrings.Ui.SHARE))
}

/**
 * Saves JPEG bytes to `Pictures/Trigger` via MediaStore.
 *
...
suspend fun Context.saveJpegToGallery(bytes: ByteArray): Boolean = withContext(Dispatchers.IO) {
    try {
        ...
        val name = "Trigger_${System.currentTimeMillis()}.jpg"
        val values = android.content.ContentValues().apply {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                put(
                    ...
                    Environment.DIRECTORY_PICTURES + "/Trigger",
                )
            }
        }
...
fun isExportableProfileImageUrl(imageUrl: String): Boolean {
    if (imageUrl.isBlank() || imageUrl == TriggerStrings.Defaults.PROFILE_IMAGE) return false
