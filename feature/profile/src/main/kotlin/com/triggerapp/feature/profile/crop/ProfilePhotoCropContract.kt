package com.triggerapp.feature.profile.crop
 ...
 * @property cropImageOptions Cropping UI and output options for [TriggerProfileCropActivity].
 * @author udit
 */
...
/**
 * Starts [TriggerProfileCropActivity] and parses its result (same intent extras as can hub's crop flow).
 * @author udit
 */
class ProfilePhotoCropContract : ActivityResultContract<ProfilePhotoCropInput, CropImageView.CropResult>() {
    ...
    override fun createIntent(context: Context, input: ProfilePhotoCropInput): Intent =
        Intent(context, TriggerProfileCropActivity::class.java).apply {
            putExtra(
